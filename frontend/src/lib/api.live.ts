/**
 * Live API client — talks to the running Django/DRF backend.
 *
 * This is the canonical implementation. `api.static.ts` mirrors it against a
 * prebuilt file tree for backendless (GitHub Pages) deploys; `api.ts` picks
 * between them. When an endpoint changes, change it here first.
 */
import type {
  AdminRegion,
  EarthquakeEvent,
...
/**
 * The API surface both backends must satisfy. Derived from the live client so
 * the static one cannot silently omit or reshape a method.
 */
export type GempaApi = typeof liveApi;
